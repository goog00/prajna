...
inline std::shared_ptr<ir::Function> ResolveKernelFunction(std::shared_ptr<ir::Value> value) {
    ...
    if (auto local = Cast<ir::LocalVariable>(value)) {
        ...
        PRAJNA_ASSERT(iter != local->instruction_with_index_list.rend());
        auto ir_write_variable_liked = Cast<ir::WriteVariableLiked>(Lock(iter->instruction));
...
inline void ConvertLLVMIntrinsicToNVVMLibdevice(std::shared_ptr<ir::Module> ir_nvptx_module) {
    ...
    std::map<std::string, std::string> name_map = {
        {"llvm.sin.f32", "__nv_sinf"},   {"llvm.cos.f32", "__nv_cosf"},
        {"llvm.tan.f32", "__nv_tanf"},   {"llvm.asin.f32", "__nv_asinf"},
...
inline void ConvertLLVMIntrinsicToAmdGPULibdevice(std::shared_ptr<ir::Module> ir_amdgpu_module) {
    ...
    std::map<std::string, std::string> name_map = {
        {"llvm.sin.f32", "__ocml_sin_f32"},   {"llvm.cos.f32", "__ocml_cos_f32"},
        {"llvm.tan.f32", "__ocml_tan_f32"},   {"llvm.asin.f32", "__ocml_asin_f32"},
